...
> **⚠️ 维护模式通知**：LiMa CLI 已进入维护模式，不再进行活跃功能开发。
> 推荐使用 [OpenCode](https://opencode.ai) 作为前端编码客户端，配合 LiMa Server 后端，
> 获得更强大的终端编码体验（LSP 支持、多会话并行、桌面 App、IDE 插件）。
> 详见 [OpenCode + LiMa 集成指南](https://github.com/zhuguang-ZFG/QWEN3.0/blob/main/docs/opencode-integration.md)。
