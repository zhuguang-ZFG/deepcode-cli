...
在 `settings.json` 中配置 `notify` 字段，指向一个可执行脚本的完整路径。每次 AI 助手完成任务应答后，LiMa 会执行该脚本，并通过环境变量注入上下文信息。
...
编辑 `~/.lima/settings.json`，添加 `notify` 字段：
...
#!/bin/bash
# macOS 系统通知
osascript -e "display notification \"任务已${STATUS:-完成}，耗时 ${DURATION}s\" with title \"LiMa\""
```
...
notify-send "LiMa" "任务已${STATUS:-完成}，耗时 ${DURATION}s"
```
