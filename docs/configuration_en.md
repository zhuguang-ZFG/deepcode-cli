...
LiMa uses the `settings.json` file for persistent configuration, supporting two storage locations:

| File Type           | Location                                  | Scope                                                                 |
| ------------------- | ----------------------------------------- | --------------------------------------------------------------------- |
| User settings file  | `~/.lima/settings.json`               | Applies to all LiMa sessions for the current user.               |
| Project settings file | `<project root>/.lima/settings.json` | Takes effect only when running LiMa in that specific project. Project settings override user settings. |

Legacy `~/.deepcode/settings.json` and `<project root>/.deepcode/settings.json` files are still read as fallbacks. New writes use `.lima` unless a project already has only a legacy `.deepcode/settings.json`.
...
LiMa has a built-in, free-to-use Web Search tool. If you need custom search logic, set `webSearchTool` to the full path of an executable script:
...
> When `command` is `npx`, LiMa automatically prepends `-y` to the arguments.
...
1. `env` defined at the top level of `settings.json` – this is a general configuration for the entire tool and all its subprocesses (global variables). Can be overridden by outer environment variables, but the environment variable KEY has the `LIMA_` prefix removed.
...
`DEEPCODE_*` environment variables are still accepted as legacy fallbacks. If both forms are present, `LIMA_*` wins.
...
4. System environment variable: `LIMA_API_KEY=abc123 lima`
...
6. System environment variable: `LIMA_THINKING_ENABLED=true lima`
...
4. System environment variable: `LIMA_WEBHOOK=... lima`
...
5. System environment variable: `LIMA_MCP_GITHUB_PERSONAL_ACCESS_TOKEN=... lima`
