...
LiMa 可以把 LiMa 当作 OpenAI-compatible 模型供应方使用。这个组合里，LiMa 负责终端 coding workflow 和 vibe coding 体验，LiMa 负责模型路由、后端健康、长期记忆和安全策略。
...
这个接入方式适合先快速跑通 LiMa + LiMa，而不是把 LiMa 直接合进 LiMa 仓库。

## 用户级配置

创建或更新 `~/.lima/settings.json`：
...
旧的 `~/.deepcode/settings.json` 仍可作为 fallback 使用，但新的 LiMa profile 应使用 `.lima`。
...
在沙盒仓库里创建 `<project>/.lima/settings.json`：
...
- 不要把 `.env`、token、VPS 凭据或 provider key 写入 LiMa 项目配置。
- GitHub、浏览器、数据库 MCP 先不要启用，等 LiMa + LiMa 基础流程稳定后再说。
- `bash` 是真实本地执行工具，高风险命令必须人工确认。

## 下一步推荐集成

基础 provider profile 跑通后，再加 LiMa result adapter，把 LiMa 任务输出归一成：
